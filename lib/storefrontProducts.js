...
export function getAvailableQuantity(product) {
  ...
  return Math.max(0, quantity - (Number.isFinite(reservedQuantity) ? reservedQuantity : 0));
}
...
export async function getStorefrontProducts() {
  ...
  if (inventoryProducts.length > 0) {
    return inventoryProducts.map(normalizeStorefrontProduct);
  }

  const webProducts = await WebProduct.find({}).sort({ createdAt: -1 }).lean();
  return webProducts.map(normalizeStorefrontProduct);
}

export async function getStorefrontProductById(productId) {
  const inventoryProduct = await Product.findById(productId).lean();
  if (inventoryProduct && inventoryProduct.productType !== "room") {
    return normalizeStorefrontProduct(inventoryProduct);
  }
