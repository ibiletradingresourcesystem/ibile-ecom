 ...
 * database never holds anything that can act on an account by itself.
 */
export const PASSWORD_RESET_TTL_MINUTES = 60;

// Verification links sit in an inbox far longer than a reset link, so they get
// a generous window but grant nothing beyond confirming the address.
...
function createOneTimeToken(ttlMs) {
...
export function createPasswordResetToken() {
  return createOneTimeToken(PASSWORD_RESET_TTL_MINUTES * 60 * 1000);
}

export function createEmailVerificationToken() {
  return createOneTimeToken(EMAIL_VERIFICATION_TTL_HOURS * 60 * 60 * 1000);
}
